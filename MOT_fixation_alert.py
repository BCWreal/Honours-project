...
class EyeLinkSession:
	...
	def setup_graphics(self, win, fix_window_deg=3.0):
...
if __name__ == '__main__':
	...
	myDlg.addField('Reverse order (Part 2 first)', False)
	dlgLabelsOrdered.append('reverseOrder')
	...
	if myDlg.OK:
		...
		reverseOrder = bool(thisInfo[dlgLabelsOrdered.index('reverseOrder')])
	else:
	...
	fixationWarningText = visual.TextStim(
		myWin,
	...
	def run_part(part_name, trial_list):
		...
		if practice_trials:
			if tracker.is_active():
				...
				print(f'No eye tracker active; starting practice trials for {part_name} immediately.')
				show_text_screen(
					myWin,
					[
						f'{part_name} practice starting now.',
						'',
						'Press SPACE to begin the practice trials.',
					],
				)
			...
			while True:
				...
				for pt_idx, practice_trial in enumerate(practice_trials):
					...
					for frameN in range(trialDurFrames):
						...
						if fix_broke:
							if trial_outcome == 'ok':
								trial_outcome = 'fixation_warning'
							if fix_break_frame == -999:
					...
					print(trialnum, subject, session, part_name, cond, basicShape, 2, speed, motion_rule, 'practice',
						  ...
						  cueFrames, timingCheckFrames,
							int(tracker.is_active()), fixatnPeriodFrames, trajectory_side, round(abs(offset_deg), 4), round(float(ellipse_rotation_rad), 4) if ellipse_rotation_rad is not None else -999,
							trial_outcome, 0, fix_break_frame, round(float(fix_break_distance_deg), 4), round(trialDurTotal, 3), 1, target_idx,
	...
	part_run_order = ['part2', 'part1'] if reverseOrder else ['part1', 'part2']
	parts_to_run_set = set(parts_to_run)
	selected_part_order = [part for part in part_run_order if part in parts_to_run_set]
	for part_index, part_key in enumerate(selected_part_order):
		run_part('Part2' if part_key == 'part2' else 'Part1', trials[part_key])
		if part_index < len(selected_part_order) - 1:
			first_name = 'Part 2' if part_key == 'part2' else 'Part 1'
			show_break_screen(f'{first_name} complete. Please take a longer break if needed. Press SPACE to continue.')
