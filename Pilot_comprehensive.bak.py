...
def stair_value_to_speed(stair_value):
	"""
	Convert PsychoPy's internal staircase value to displayed rps.

	The internal scale is inverted so that PsychoPy's 3-down step
	decreases the internal value but increases the displayed rps.
	This gives the intended rule:
	- 3 correct responses -> increase rps by one step
	- 1 incorrect response -> decrease rps by one step
	"""
	return -stair_value


def display_speed_to_stair_value(display_speed):
	"""Convert displayed rps to PsychoPy's inverted internal staircase value."""
...
def make_practice_trials(part_name, trial_specs):
	"""
	Build practice trials with predefined familiarisation speeds.

	Part 1:
		- 2 practice trials per condition
		- Speeds: 0.5 rps, 1.0 rps

	Part 2:
		- 3 practice trials per condition
		- Speeds: 0.5 rps, 0.7 rps, 1.0 rps
	"""
	trials = []
...
def create_staircase_window(screen_index=None, fullscr=True):
	try:
		if screen_index is None:
			screen_index, win_size = get_screen_info(prefer_second=True)
		else:
			# Get the actual pixel size of the requested visualizer screen.
			try:
				import pyglet
				display = pyglet.canvas.get_display()
				screens = display.get_screens()
				if 0 <= int(screen_index) < len(screens):
					target = screens[int(screen_index)]
					win_size = [int(target.width), int(target.height)]
				else:
					_, win_size = get_screen_info(prefer_second=True)
			except Exception:
				_, win_size = get_screen_info(prefer_second=True)
...
if __name__ == '__main__':
	...
	stairWin = create_staircase_window(screen_index=viz_screen_index, fullscr=fullscr)
